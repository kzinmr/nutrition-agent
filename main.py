...
from agents.nutrition_planner import NutritionPlannerAgent, Inventory, DietaryConstraints
...
def load_sample_data(scenario_name: str) -> Dict[str, Any]:
    ...
    data_path = Path(__file__).parent / "data" / "test_prompts" / f"{scenario_name}.json"
...
def create_interactive_inputs() -> tuple[Inventory, DietaryConstraints]:
    ...
    console.print(Panel("🥘 [bold blue]Nutrition Agent - Interactive Setup[/bold blue]"))
    ...
    if Confirm.ask("Do you have any dietary restrictions?"):
        restriction_input = Prompt.ask("List restrictions (comma-separated, e.g., vegetarian, low-carb)")
...
async def run_sample(scenario: str, model: str, days: int):
...
def save_meal_plan(meal_plans: List, shopping_list: Dict, filename: str):
