...
def run(
    scenarios_dir: str = typer.Option(
        "data/test_prompts",
        help="Directory containing test scenario JSON files"
    ),
...
async def run_evaluation(scenarios_dir: str, models: List[str], days: int, output: str):
...
async def run_single_evaluation(scenario: str, model: str, days: int):
    ...
    result = await evaluator.evaluate_scenario(
        scenario_path, model_configs[0][1], days
    )
...
async def run_model_comparison(models: List[str], output: str):
    ...
    for rank, (model, scores) in enumerate(sorted_models, 1):
        ...
        console.print(f"{rank}. {model}: {avg_score:.3f} (±{max(scores) - min(scores):.3f})")
...
def validate():
    ...
    for scenario_file in scenario_files:
        try:
            ...
            if missing_keys:
                console.print(f"[red]❌ {scenario_file.name}: Missing keys: {missing_keys}[/red]")
            else:
