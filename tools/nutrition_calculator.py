...
class NutritionCalculator:
    ...
    @staticmethod
    def calculate_nutrition_error(actual: MealNutrition, target: NutritionTarget) -> Dict[str, float]:
        """
        ...
        """
        calories_error = abs(actual.calories - target.daily_calories) / target.daily_calories * 100
        protein_error = abs(actual.protein_g - target.daily_protein_g) / target.daily_protein_g * 100
        fat_error = abs(actual.fat_g - target.daily_fat_g) / target.daily_fat_g * 100
        carbs_error = abs(actual.carbs_g - target.daily_carbs_g) / target.daily_carbs_g * 100
    ...
    def suggest_portion_adjustments(
        ...
        scale_factor = target_meal_calories / meal.calories if meal.calories > 0 else 1.0
        ...
        return {
            ...
            "meets_protein": abs(scaled_protein - target_meal_protein) / target_meal_protein < 0.1,
            "meets_fat": abs(scaled_fat - target_meal_fat) / target_meal_fat < 0.1,
            "meets_carbs": abs(scaled_carbs - target_meal_carbs) / target_meal_carbs < 0.1
        }


# Tool functions for agent use
def calculate_pfc_balance(meals: List[Dict[str, Any]], target_calories: float, target_pfc: Tuple[float, float, float]) -> Dict[str, Any]:
    """Calculate PFC balance for a set of meals and compare to targets."""
    
    # Convert meal data to MealNutrition objects
    meal_nutritions = []
    for meal in meals:
        meal_nutritions.append(MealNutrition(
            meal_name=meal['name'],
    ...
    target = NutritionTarget(
        ...
        pfc_ratio=target_pfc
    )
    
    # Calculate errors
    errors = calculator.calculate_nutrition_error(daily_nutrition, target)
    meets_constraints, violations = calculator.check_nutrition_constraints(daily_nutrition, target)
